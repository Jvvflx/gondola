...
export class MetricsService {
    ...
    async getRuptureAlerts(tenantId: string): Promise<ProductAlert[]> {
    ...
    async getExcessStockAlerts(tenantId: string): Promise<ProductAlert[]> {
        const products = await prisma.product.findMany({
            where: { tenantId, active: true },
            include: {
                stocks: true,
    ...
    async getValidityAlerts(tenantId: string): Promise<ProductAlert[]> {
        ...
        const alerts: ProductAlert[] = validities.map((validity) => {
            const daysUntilExpiration = Math.floor(
                (validity.expirationDate.getTime() - Date.now()) / (24 * 60 * 60 * 1000)
            );
    ...
    async getSalesHistory(tenantId: string, days: number = 30) {
