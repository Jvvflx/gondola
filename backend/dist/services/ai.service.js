...
class AIService {
    ...
    async generatePromotionSuggestions(tenantId) {
        ...
        for (const alert of excessStock.slice(0, 5)) {
            ...
            const aiInsight = await this.getAIInsight(`Produto: ${alert.productName}. ${alert.reason}. Estoque atual: ${alert.currentStock} unidades. Sugira uma estratégia de promoção.`);
            suggestions.push({
        ...
        for (const alert of validityAlerts.slice(0, 5)) {
            ...
            const aiInsight = await this.getAIInsight(`Produto: ${alert.productName}. ${alert.reason}. Quantidade: ${alert.currentStock} unidades. Sugira uma estratégia de promoção urgente.`);
            suggestions.push({
                productId: alert.productId,
                productName: alert.productName,
                sku: alert.sku,
